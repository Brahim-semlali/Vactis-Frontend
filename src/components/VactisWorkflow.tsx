...
function EngineCanvas({
  ...
  useEffect(() => {
    ...
    const render = (now: number) => {
      ...
      // A colored moving pulse keeps the flow visible even between particles.
      if (!reduced) {
        ctx.save()
        trace(paths.main.pts)
        ctx.setLineDash([22, 72])
        ctx.lineDashOffset = -(now / 10) % 94
        ctx.lineWidth = 5
      ...
      // --- return loop (monthly cycle): colored flow returning to the input
      ctx.save()
      const returnGradient = ctx.createLinearGradient(0, height, 0, 0)
      returnGradient.addColorStop(0, "rgba(13,148,136,0.68)")
      returnGradient.addColorStop(0.42, "rgba(37,99,235,0.46)")
      returnGradient.addColorStop(0.78, "rgba(8,169,165,0.4)")
      returnGradient.addColorStop(1, "rgba(13,148,136,0.62)")
      ctx.setLineDash([5, 8])
      ctx.lineDashOffset = reduced ? 0 : -(now / 18) % 13
      trace(paths.ret.pts)
      ctx.lineWidth = 3
      ctx.strokeStyle = returnGradient
      ctx.stroke()
      if (!reduced) {
        trace(paths.ret.pts)
        ctx.setLineDash([16, 58])
        ctx.lineDashOffset = -(now / 13) % 74
        ctx.lineWidth = 4
      ...
      // --- directional flow markers: make the circulation readable at a glance
      const headCount = 7
      for (let i = 0; i < headCount; i++) {
        const phase = reduced ? i / headCount : (now / 5200 + i / headCount) % 1
        let head: Pt
        let color: [number, number, number]
        if (phase < paths.mainFrac) {
          const local = phase / paths.mainFrac
          head = pointAtLength(paths.main, local * paths.main.len)
          color = accentAt(local)
        } else {
          const local = (phase - paths.mainFrac) / (1 - paths.mainFrac)
          head = pointAtLength(paths.ret, local * paths.ret.len)
          color = [13, 148, 136]
        }
        const [r, gg, b] = color
        const glow = ctx.createRadialGradient(head.x, head.y, 0, head.x, head.y, 18)
        glow.addColorStop(0, `rgba(${r},${gg},${b},0.8)`)
      ...
      <div className="relative p-5 sm:p-8">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          ...
          >
            ...
            {playing ? "Quitter le parcours" : "Explorer le parcours"}
            {!playing && (
        ...
        >
          ...
          {!playing && (
            <div className="pointer-events-none absolute right-3 top-3 z-10 flex items-center gap-2 rounded-full border border-slate-200 bg-white/80 px-3 py-1.5 text-xs text-slate-500 backdrop-blur">
              ...
              <span>Cliquez une étape ou explorez le parcours</span>
            </div>
          )}
        </div>
